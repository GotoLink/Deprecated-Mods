...
public abstract class HoverboatProjectileType implements Comparable<HoverboatProjectileType> {
	...
	static {
		...
		HoverboatProjectileType._NameSpecialExceptions.put(EntityEgg.class, "Egg");
		HoverboatProjectileType._NameSpecialExceptions.put(EntityFireball.class, "Fireball");
	}
	...
	public boolean settingBoolDropEnabled;
	public boolean settingBoolProjectileEnabled;
	...
	public Entity DropItem(World world, EntityLiving entityliving, double d, double d1, double d2, float f) {
		try {
			...
			if (entity.riddenByEntity != null) {
				world.spawnEntityInWorld(entity.riddenByEntity);
			}
			if (usesCustomSounds) {
	...
	public Entity FireItem(World world, EntityLiving entityliving, double d, double d1, double d2, float f, float f1) {
		try {
			...
			if (!throwable) {
				...
				Point2D point2d = Hoverboat.RotatePoint(new java.awt.geom.Point2D.Double(0.0D, 1.0D), new java.awt.geom.Point2D.Double(), Hoverboat.Degreetonormalizedradian(f1));
				d3 = mod_Hoverboat.Instance.settingFloatTNTVel;
				entity.motionY = point2d.getX();
				point2d = Hoverboat.RotatePoint(new java.awt.geom.Point2D.Double(0.0D, point2d.getY()), new java.awt.geom.Point2D.Double(), Hoverboat.Degreetonormalizedradian(f));
				...
				if (usesCustomSounds) {
					...
					world.playSoundAtEntity(entityliving, "random.explode", 1.0F, 1.5F + ((world.rand.nextFloat() - 0.5F) * 0.4F));
				}
			} else {
				d3 = mod_Hoverboat.Instance.settingFloatArrowVel;
				if (usesCustomSounds) {
					PlayCustomSound(false, entity);
				} else {
					world.playSoundAtEntity(entityliving, "random.bow", 0.3F, 1.0F / ((world.rand.nextFloat() * 0.6F) + 0.7F));
				}
			}
			entity.motionX *= d3;
			entity.motionY *= d3;
	...
	public String GetName() {
		...
		try {
			if (HoverboatProjectileType._NameSpecialExceptions.containsKey(ClassType)) {
				name = HoverboatProjectileType._NameSpecialExceptions.get(ClassType);
				return name;
			}
			if ((Entity.class).isAssignableFrom(ClassType)) {
				name = EntityList.getEntityString((Entity) ClassType.getConstructor(new Class[] { World.class }).newInstance(new Object[] { ModLoader.getMinecraftInstance().theWorld }));
				return name;
			}
		} catch (Exception exception) {
			exception.equals(null);
		}
		return name = (new StringBuilder("NO NAME FOUND (")).append(ClassType.toString()).append(")").toString();
	}
