...
public class Hoverboat extends Entity implements IInventory {
	...
	public boolean attackEntityFrom(DamageSource source, int damage) {
		...
		if (CurrentDamage > 80) {
			...
			if (source.getSourceOfDamage() instanceof EntityPlayer) {
				if (!((EntityPlayer) source.getSourceOfDamage()).inventory.addItemStackToInventory(new ItemStack(mod_Hoverboat.hoverboatitem.shiftedIndex, 1, 0))) {
					dropItemWithOffset(mod_Hoverboat.hoverboatitem.itemID, 1, 0.0F);
				}
			} else {
				dropItemWithOffset(mod_Hoverboat.hoverboatitem.itemID, 1, 0.0F);
			}
			for (int j = 0; j < inventory.length; j++) {
				if (inventory[j] != null) {
					dropItemWithOffset(inventory[j].itemID, inventory[j].stackSize, 0.0F);
					inventory[j] = null;
				}
			}
			setDead();
	...
	public void onUpdate() {
		...
		double MaxSpeed = mod_Hoverboat.Instance.settingFloatMaxSpeed;
		boolean SubEnabled = mod_Hoverboat.Instance.settingBoolSub;
		isImmuneToFire = mod_Hoverboat.Instance.settingBoolFireProof;
		...
		if (riddenByEntity != null) {
			...
			case 1: // Free View
			{
				rotate = (KeySteeringLast - (PlayerLR * AltTurnSpeed)) * (PlayerLR != 0.0F ? 0.1F : 0.3F);
				...
				Point2D point2d = Hoverboat.RotatePoint(new java.awt.geom.Point2D.Double(motionX, motionZ), new java.awt.geom.Point2D.Double(), Hoverboat.Degreetonormalizedradian(-rotate));
				motionX = point2d.getX();
				motionZ = point2d.getY();
				if (PlayerFB != 0.0F) {
					Point2D point2d1 = Hoverboat.RotatePoint(new java.awt.geom.Point2D.Double(-(PlayerFB * 0.01D), 0.0D), new java.awt.geom.Point2D.Double(),
							Hoverboat.Degreetonormalizedradian(rotationYaw - rotate));
			...
			case 2: // Flight Style
			{
				...
				rotate = Hoverboat.NormalizeDegree(rotationYaw - (riddenByEntity.rotationYaw - 90D)) * AltTurnSpeedDamper;
				...
				Point2D point2d2 = Hoverboat.RotatePoint(new java.awt.geom.Point2D.Double(motionX, motionZ), new java.awt.geom.Point2D.Double(), Hoverboat.Degreetonormalizedradian(-rotate));
				motionX = point2d2.getX();
				motionZ = point2d2.getY();
				point2d2 = Hoverboat.RotatePoint(new java.awt.geom.Point2D.Double(-(PlayerFB * 0.01D), 0.0D), new java.awt.geom.Point2D.Double(),
						Hoverboat.Degreetonormalizedradian(rotationYaw - rotate));
			...
			case 3: // beta style
			{
				...
				Point2D point2d3 = Hoverboat.RotatePoint(new java.awt.geom.Point2D.Double(motionX, motionZ), new java.awt.geom.Point2D.Double(), Hoverboat.Degreetonormalizedradian(-rotate));
				motionX = point2d3.getX();
				motionZ = point2d3.getY();
				if (PlayerFB != 0.0F) {
					Point2D point2d4 = Hoverboat.RotatePoint(new java.awt.geom.Point2D.Double(-(PlayerFB * 0.01D), 0.0D), new java.awt.geom.Point2D.Double(),
							Hoverboat.Degreetonormalizedradian(rotationYaw - rotate));
					motionX += point2d4.getX() * AccelerationMultiplier;
					motionZ += point2d4.getY() * AccelerationMultiplier;
				}
				if (PlayerLR != 0.0F) {
					Point2D point2d5 = Hoverboat.RotatePoint(new java.awt.geom.Point2D.Double(-(PlayerLR * 0.01D), 0.0D), new java.awt.geom.Point2D.Double(),
							Hoverboat.Degreetonormalizedradian(rotationYaw - rotate - 90D));
			...
			if (SpeedLevel > 0.7D) {
				hoverheight = mod_Hoverboat.Instance.settingFloatMaxHover;
			}
			if (mod_Hoverboat.Instance.settingBoolAllowFastFov) {
				if (CurrentSpeed > 0.2) {
			...
			if (PlayerJumping) {
				if (CurrentMode != 0) {
					if ((currentheight < (hoverheight + 0.5D)) || ((CurrentMode == 2) && (CurrentSpeed > (MaxSpeed * 0.5D)))) {
						extrajump = 0.6D;
				...
				} else if (!java.lang.Double.isNaN(depth) && SubEnabled && (depth > 0.0D)) {
			...
			if (SlowDecent && (currentheight > (hoverheight + 2D)) && (extrajump == 0.0D)) {
			...
			hoverheight = mod_Hoverboat.Instance.settingFloatMinHover;
			motionX *= 0.8D;
			motionZ *= 0.8D;
		}
		double wobble = (Math.sin((System.currentTimeMillis() % 360000) / 200D) - 1.0D) * 0.0078125D;
		if (CurrentMode != 0) {
			double working = 0.0D;
			if (currentheight < hoverheight) {
				working = 0.2D * ((hoverheight - currentheight) * 0.8D);
			}
			if ((Math.abs(working * 100000D) > 0.0D) || (Math.abs(extrajump * 100000D) > 0.0D)) {
			...
			double limiter = (lastDive + 2000L) <= System.currentTimeMillis() ? 0.09D : 0.013D;
			...
			motionY -= java.lang.Double.isNaN(depth) ? 0.04D : 0.005D;
		}
		...
		if ((CurrentSpeed < mod_Hoverboat.Instance.settingFloatBrakeThreshold) && (0.0F > PlayerFB) && (KeyboardSteering != 3) && (KeyboardSteering != 2)) {
		...
		motionX *= mod_Hoverboat.Instance.settingFloatMaxFriction;
		motionY *= 0.95D;
		motionZ *= mod_Hoverboat.Instance.settingFloatMaxFriction;
		rotationPitch = 0.0F;
		double Rotation = rotationYaw;
		double XChange = prevPosX - posX;
		double ZChange = prevPosZ - posZ;
		if ((CurrentSpeed > 0.001D) && (((KeyboardSteering != 3) && (KeyboardSteering != 2)) || (riddenByEntity == null))) {
			XChange *= 1000D;
			ZChange *= 1000D;
		...
		float lean = 0.0F;
		if (Rotation < 0.0D) {
			lean = (float) (-Math.pow(Math.pow(-Rotation * 20D, SpeedLevel), SpeedLevel));
		} else {
			lean = (float) Math.pow(Math.pow(Rotation * 20D, SpeedLevel), SpeedLevel);
		}
		...
		for (int i = 0; i < ((1.0D + (SpeedLevel * (java.lang.Double.isNaN(depth) ? 10D : -1D))) * mod_Hoverboat.Instance.settingFloatParticles); i++) {
			Point2D point2d3 = Hoverboat.RotatePoint(new java.awt.geom.Point2D.Double((rand.nextDouble() - 0.5D) * 1.3D, (rand.nextDouble() - 0.5D) * 1.3D), double1, sinrot, cosrot);
			String s = (java.lang.Double.isNaN(depth) || isDivingInLava ? (rand.nextBoolean() && (SpeedLevel > 0.98) ? "explode" : "smoke") : "splash");
			worldObj.spawnParticle(s, prevPosX + point2d3.getX(), prevPosY - 0.3D, prevPosZ + point2d3.getY(), point2d3.getX() * 0.02D * (1.0D + SpeedLevel), !isDivingInLava
					&& (currentheight >= 0.5D) ? -0.04D * hoverheight : 0.01D, point2d3.getY() * 0.02D * (1.0D + SpeedLevel));
		}
	...
	public void updateRiderPosition() {
		...
		double shift = mod_Hoverboat.Instance.settingFloatViewShift;
		double x = 0.0D;
		double z = 0.0D;
		if (shift != 0.0D) {
		...
		riddenByEntity.setPosition(posX + x, posY + getMountedYOffset() + riddenByEntity.getYOffset(), posZ + z);
	}
	...
	protected void readEntityFromNBT(NBTTagCompound nbttagcompound) {
		...
		SelectedArrow = mod_Hoverboat.Instance.GetProjectileFromString(nbttagcompound.getString("SelectedProj"));
		if (SelectedArrow == null) {
			SelectedArrow = mod_Hoverboat.Instance.GetDefaultProjectile();
		}
		SelectedDrop = mod_Hoverboat.Instance.GetDropFromString(nbttagcompound.getString("SelectedDrop"));
	...
	private void CheckKeys() {
		...
		if (!mod_Hoverboat.Instance.GetValidProjectile(SelectedArrow)) {
			SelectedArrow = mod_Hoverboat.Instance.GetNextProjectile(SelectedArrow);
		}
		...
		if (mod_Hoverboat.Instance.settingKeyBrake.isKeyDown()) {
			motionX *= mod_Hoverboat.Instance.settingFloatBrakeMult;
			motionZ *= mod_Hoverboat.Instance.settingFloatBrakeMult;
		}
		...
		if (GetKeyLocked(mod_Hoverboat.Instance.settingKeyShiftArrowLeft)) {
			SelectedArrow = mod_Hoverboat.Instance.GetPrevProjectile(SelectedArrow);
		}
		...
		if (mod_Hoverboat.Instance.settingKeyFireTnt.isKeyDown() && (CurrentMode == 2)) {
			DropTnT();
		}
		if (GetKeyLocked(mod_Hoverboat.Instance.settingKeyPark)) {
			IsParked = !IsParked;
		}
		if ((lastPressedKey != -1) && !Keyboard.isKeyDown(lastPressedKey) && keyDown) {
			lastPressedKey = -1;
			keyDown = false;
		}
	}
			...
			for (int l = 0; l < inventory.length; l++) {
				if (inventory[l] != null) {
					...
					if (j1 != 1) {
						...
						} while ((l1 == -1) || (inventory[l1].stackSize >= j1) || (l1 == l));
						if (l1 != l) {
							int i2 = inventory[l1].stackSize + inventory[l].stackSize;
							if (i2 > j1) {
		...
		for (int k1 = 0; k1 < inventory.length; k1++) {
			if (inventory[k1] != null) {
				if ((i1 != -1) && (i1 < i)) {
					...
					dropItemWithOffset(inventory[k1].itemID, inventory[k1].stackSize, 0.0F);
					inventory[k1] = null;
				}
			}
		}
	...
	private void DropTnT() {
		if (!mod_Hoverboat.Instance.settingBoolEnableTNT) {
			return;
		}
		MakeDrops(mod_Hoverboat.Instance.settingIntTntLines);
		boolean useammo = mod_Hoverboat.Instance.settingBoolUseAmmo;
		int tntFireRate = mod_Hoverboat.Instance.settingIntTNTFireRate;
		...
		for (int j = 0; j < dropPoints.length; j++) {
			if (((Long) dropPoints[j][1]).longValue() <= time) {
				Point2D point2d = Hoverboat.RotatePoint((Point2D) dropPoints[j][0], double1, sin, cos);
	...
	private void FireArrow() {
		if ((nextProjectile >= System.currentTimeMillis()) || !mod_Hoverboat.Instance.settingBoolEnableArrows) {
			return;
		}
		if (mod_Hoverboat.Instance.settingBoolUseAmmo && !SelectedArrow.UseItem(this)) {
			return;
		}
		double x = posX;
		double y = (riddenByEntity.posY + riddenByEntity.getEyeHeight()) - 0.3D;
		double z = posZ;
		Point2D point2d = Hoverboat.RotatePoint(new java.awt.geom.Point2D.Double(0.0D, 3.5D), new java.awt.geom.Point2D.Double(), Hoverboat.Degreetonormalizedradian(riddenByEntity.rotationPitch));
		y += point2d.getX();
		point2d = Hoverboat.RotatePoint(new java.awt.geom.Point2D.Double(0.0D, point2d.getY()), new java.awt.geom.Point2D.Double(), Hoverboat.Degreetonormalizedradian(riddenByEntity.rotationYaw));
		x += point2d.getX();
		z += point2d.getY();
		SelectedArrow.FireItem(worldObj, (EntityLiving) riddenByEntity, x, y, z, riddenByEntity.rotationYaw + (float) (rand.nextGaussian() * mod_Hoverboat.Instance.settingFloatCannonAccuracy),
				riddenByEntity.rotationPitch + (float) (rand.nextGaussian() * mod_Hoverboat.Instance.settingFloatCannonAccuracy));
		nextProjectile = System.currentTimeMillis() + mod_Hoverboat.Instance.settingIntArrowFireRate;
		return;
	}
	...
	private void MakeDrops(int i) {
		...
		for (int j = 0; j < dropPoints.length; j++) {
			x += part;
			dropPoints[j][0] = new java.awt.geom.Point2D.Double(x, -(2D + (Math.abs(x) / 2.5D)));
	...
	public static Point2D RotatePoint(Point2D point2d, Point2D point2d1, double d) {
		return Hoverboat.RotatePoint(point2d, point2d1, Math.sin(d), Math.cos(d));
	}
